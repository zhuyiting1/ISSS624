...
The correlation plot above shows that `COMPUTER_PR` and `INTERNET_PR` are highly correlated (R^2^ = 0.87 \> 0.85). This suggest that only one of them should be used in the cluster analysis instead of both. In this exercise, we will use `COMPUTER_PR` instead of `INTERNET_PR` as the former provides direct output and interaction with users, similar to other ICT variables in this data set.
